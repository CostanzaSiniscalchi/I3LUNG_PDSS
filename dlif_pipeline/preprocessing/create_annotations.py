...
def create_feature_dataset_from_processed(
    ...
    for split in ['train', 'test', 'ext_val']:
        ...
        # Drop SET and CENTER from all modalities
        cols_to_drop = ['Subject', 'SET', 'CENTER']
        ...
        df_split['mod1'] = rwd_features.apply(lambda r: r.tolist(), axis=1)
        
        # Add mod2 (radiomics) - None if subject not in rad
        df_split['mod2'] = df_split['Subject'].apply(
            lambda s: rad_features.loc[rad[rad['Subject'] == s].index].values[0].tolist() 
        ...
        df_split['mod3'] = df_split['Subject'].apply(
            lambda s: dp_features.loc[dp[dp['Subject'] == s].index].values[0].tolist() 
        ...
        df_split['mod4'] = df_split['Subject'].apply(
            lambda s: gen_features.loc[genomics[genomics['Subject'] == s].index].values[0].tolist() 
